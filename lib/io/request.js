 ...
 * otherwise). Plus the GLOBAL ACTIVE-IO ledger: IO alone owns the
...
export async function runRequest(aiio, context, callbacks, options) {
  ...
  try {
    ...
    aiio._state = "sending";
    const connection = new aiio.Provider(aiio.url, aiio);
    ...
    aiio._state = "reading";
    clearTimeout(connectTimer); // the response started: connection is fine
