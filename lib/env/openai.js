...
/**
 * The documented Responses API `web_search` tool (verified against
 * platform.openai.com/docs/guides/tools-web-search on 2026-09-22:
 * `tools: [{"type":"web_search"}]`, result read from
 * `response.output_text`). Hosted ONLY on api.openai.com itself — the
 * Codex backend (chatgpt.com/backend-api/codex), LM Studio, and other
 * Responses-shaped endpoints have no such server tool, so they answer
 * undefined (honest fall-through), never a request that could only
 * 400. `web.provider === false` opts out at the app-settings level
 * (aiio.settings is only the endpoint's namespaced view — the gate
 * reads aiio.env.settings, the full app settings tree).
 */
export async function webSearch({ aiio, args, signal, deadline }) {
  if (aiio?.env?.settings?.web?.provider === false) return undefined;
  if (String(aiio?.url ?? "").replace(/\/$/, "") !== "https://api.openai.com/v1") return undefined;
  ...
  try {
    const response = await fetch(`${String(aiio.url).replace(/\/$/, "")}/responses`, {
      method: "POST",
      headers: { "content-type": "application/json", authorization: `Bearer ${token}` },
      signal: controller.signal,
      body: JSON.stringify({
        model: aiio.currentModel,
        input: `Search the web for "${query}" and answer with the top results as a Markdown list: each result one bullet with its title, URL, and a one-or-two-sentence snippet.`,
    ...
    // message items carry the visible answer as output_text content
    const parts = [];
