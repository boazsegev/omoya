 ...
 * passed a SHADOW endpoints map (existing dynamic entries hidden): a
...
export async function detectEndpoints(env, { timeout = 300 } = {}) {
  return env.batch(async () => {
    ...
    const shadow = Object.fromEntries(Object.entries(env.endpoints)
      .filter(([name]) => !env._dynamicEndpoints.has(name)));
    ...
    for (const [name, ep] of candidates) {
      if (env._configuredEndpoints?.has(name)) continue; // a settings.providers entry always wins
      const existing = env.endpoints[name];
      if (!env._dynamicEndpoints.has(name) && existing !== undefined) {
