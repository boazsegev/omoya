 ...
 * execution. It is honored only for tools loaded from the package or
 * a namespace system root; other roots cannot grant themselves trust.
...
export function defaultToolRoots(env) {
  ...
  // The system root is an administrator-controlled executable-tool layer.
  // Unlike ordinary configured roots, tools from here may opt into
  // unrestricted host execution with schema `trusted: true`.
  if (process.env[NAMES.systemEnv]) {
    roots.push(...process.env[NAMES.systemEnv].split(delimiter).filter(Boolean).map((root) => join(root, "tools")));
  }
...
export async function refreshTools(env) {
  ...
  if (process.env[NAMES.systemEnv]) {
    trustedRoots.push(...process.env[NAMES.systemEnv].split(delimiter).filter(Boolean).map((root) => join(root, "tools")));
  }
