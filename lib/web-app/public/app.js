...
let settings = { safe: false, thinking: "default", sessionSave: undefined, endpoint: null, model: null, models: [] };
...
function handle(m) {
  switch (m.type) {
    ...
    case "sessions":
      ...
      if (agent) { const mine = agentList().find((item) => item.id === agent.id); if (mine) agent = { ...agent, busy: mine.state === "working", state: mine.state }; }
      updateSidebar(); updateHeader(); updateComposerActivity(); break;
    case "agent":
      if (agent && m.agent?.id === agent.id) {
        agent = m.agent;
        ...
        updateHeader(); updateComposerActivity();
      }
      break;
    case "settings":
      settings = {
    ...
    case "turn.start":
      ...
      updateUsage(); updateHeader(); updateComposerActivity();
      break;
...
function onTurnEnd(m) {
  ...
  if (agent && typeof m.busy === "boolean") agent = { ...agent, busy: m.busy, state: m.busy ? "working" : "idle" };
  updateUsage();
  updateHeader();
...
// Composer height follows content, capped at eight rows.
function autofit() {
  if (!textareaEl) return;
  textareaEl.style.height = "auto";
  textareaEl.style.height = Math.min(textareaEl.scrollHeight, 8 * 24) + "px";
}
...
function updateHeader() {
  ...
  chips.append(think);
  // Session logging is always toggleable: persisted agents flip their
  // store's save flag; an anonymous agent opts in by starting a saved
  // session (the server performs the newSession handoff).
  const logging = el("button", "chip" + (settings.sessionSave ? " active" : ""), settings.sessionSave ? "📝 Logging on" : "📝 Logging off");
  logging.type = "button";
  logging.setAttribute("aria-label", "Toggle session logging");
  logging.title = settings.sessionSave ? "Turn off session logging (memory only)" : "Turn on session logging";
  logging.addEventListener("click", () => send({ type: "settings.session-save", on: settings.sessionSave !== true }));
  chips.append(logging);
...
function buildComposer() {
  ...
  const auto = autofit;
  textareaEl.addEventListener("input", () => { noteComposerInput(); auto(); updateAutocomplete(); });
  textareaEl.addEventListener("keydown", (e) => {
    if (autocompleteKey(e)) return;
    if ((e.key === "ArrowUp" || e.key === "ArrowDown") && !e.shiftKey && recallHistory(e.key === "ArrowUp" ? -1 : 1)) { e.preventDefault(); return; }
    if (e.key === "Enter" && !e.shiftKey && !acOpen()) { e.preventDefault(); form.requestSubmit(); }
  });
  form.addEventListener("submit", (e) => {
    ...
    const draft = composerDraft();
    if (text) draft.submitted.push(text);
  ...
  setTimeout(() => { auto(); textareaEl.focus(); }, 0);
  return form;
}
...
async function addFiles(files) {
  ...
  for (const file of [...(files ?? [])]) {
    ...
    try {
      ...
      if (composerDraft() === draft) { draftAttachments = [...draft.attachments]; renderAttachmentChips(); }
...
function renderAttachmentChips() {
  ...
  for (const [index, file] of draftAttachments.entries()) {
    ...
    remove.addEventListener("click", () => { draftAttachments.splice(index, 1); composerDraft().attachments = [...draftAttachments]; renderAttachmentChips(); });
