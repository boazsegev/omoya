...
export class AgentSession {
  ...
  #onResponse(event, value) {
    ...
    if (event === EVENT.START) {
      ...
      this.#history = historyOf(this.#agent.context);
      this.#current = null;
      this.#send({ type: "turn.start", status: this.statusSnapshot() });
      return;
    }
    if (event === EVENT.DONE || event === EVENT.ERROR) {
      ...
      this.#current = null;
      this.#send({ type: "turn.end", terminal: { type: value.type ?? (event === EVENT.DONE ? "done" : "error"), ...(value.error ? { error: String(value.error) } : {}), ...(value.kind ? { kind: String(value.kind) } : {}) }, pendingCount: this.#agent.pending.length, status: this.statusSnapshot() });
    }
  }
