 ...
 *   session.rename     {name}                     rename the viewed agent's saved session
 *   session.clear      {}                         clear the viewed session and restart it (/session-delete!)
 *   agent.rename       {agentId?, name}           set an agent's display name (default: the viewed one)
 ...
 *   settings.theme     {name}                     persist tui.theme (shared with the TUI); "default" = system
 ...
 *   endpoints          {endpoints:[{name,models,loginRequired}], presets:[{name,label,provider,url,oauth}], removable}
...
/** A non-empty, trimmed, single-purpose string field. */
...
export function parseClientMessage(raw) {
  ...
  switch (type) {
    ...
    case "session.rename": return { type, name: name(value.name, "name") };
    case "agent.rename": return { type, name: name(value.name, "name"), ...(value.agentId === undefined ? {} : { agentId: string(value.agentId, "agentId", 512) }) };
    ...
    case "endpoint.oauth": case "endpoint.logout": return { type, name: name(value.name, "name") };
