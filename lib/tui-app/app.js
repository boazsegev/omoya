...
const { loginEndpoint, writeLastCombo } = CLI;
 ...
 */
export function createApp(agent, { env, cwd = "", sources, completionIO, log, open } = {}) {
  ...
  function finishEndpointLogin(result) {
    const current = slot.current();
    current.endpoint = result.name;
    const models = current.env.endpointSettings(result.name).models ?? {};
    current.model = Object.keys(models).find((id) => models[id]?.secret !== true);
    writeLastCombo(current.env, { endpoint: current.endpoint, model: current.model });
  }
