...
/** Descend into a sub-menu (a selected item that lists more items). No
 *  overlay is open yet when this is reached as a direct entry point
 *  (e.g. typing `/endpoint-login` at the prompt rather than through the
 *  ^X menu) — start a fresh one instead of crashing on a null stack. */
export function pushMenu(overlay, title, items) {
  return overlay ? { ...overlay, stack: [...overlay.stack, { title, items }] } : openMenu(title, items);
}
