 ...
 * model ids. The refresh is Env's single bounded fan-out (refreshModels);
 * the listing itself reads the cache back through the same published-endpoint
 * filter and secret-strip as the menu (listEndpointModels), so a provider
 * failure shows that endpoint's cached/static catalogue rather than holding
 * the listing hostage to the OS TCP timeout. */
export async function listModels(env, { access = env.settings?.modelAccess ?? "all", timeout } = {}) {
  await env.refreshModels(timeout === undefined ? {} : { timeout });
  return listEndpointModels(env, { access }).map(({ name, models }) => ({ name, models }));
}
