...
export function readLastCombo(env) {
  try {
    const parsed = JSON.parse(readFileSync(join(env.settingsDir ?? env.dir, LAST_COMBO_FILE), "utf8"));
    if (typeof parsed?.model !== "string" || parsed.model === "") return null;
    const combo = { model: parsed.model };
    if (typeof parsed.endpoint === "string" && parsed.endpoint !== "") combo.endpoint = parsed.endpoint;
    return combo;
  } catch {
